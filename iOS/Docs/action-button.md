...
- **Restarts that stay.**
  - *Silent first second* (the whole first second below −90 dBFS): **once** per recording, and only in the foreground, the recorder drops the silence, deactivates the session, sets it up and activates it again, and restarts the same engine — exactly what the next recording did on the first device run, where it worked. If it is silent again, it is only logged, and the stop reports `failed.silentInput`. In the background it is only logged (a non-mixable session can't be re-activated there).
  ...
  - *Dropped:* starting over on a new engine, and starting over when an input route “appears” after activation (that route change is a normal part of activating the session; acting on it tore down the engine at 0 s).
...
| `diagnostic.audioRestart` | a silent first second, an engine configuration change, or a media-services reset | reason and result |
