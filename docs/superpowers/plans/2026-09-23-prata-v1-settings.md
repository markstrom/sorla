# Prata v1 Settings, Indicator, Clipboard, Login Item — Implementation Plan
...
**Spec:** [docs/superpowers/specs/2026-09-23-prata-design.md](../specs/2026-09-23-prata-design.md) §13–§14.

**Existing code (branch `walking-skeleton`):** `PrataCore` — `AudioRecorder`, `ParakeetTranscriptionEngine` (actor, `SpeechModel`-selected), `RecordingController` (`@MainActor`, 150 ms tail, `cancelRecording()`, `setEngine(_:name:)`, os.Logger), `PasteService`, `PermissionsManager`, `PushToTalkGesture` (pure state machine), `RightCommandKeyMonitor` (NSEvent monitors), `SpeechModel`. `Prata` — `AppDelegate` (status item menu with model picker, `UserDefaults` key `selectedModel`), `main.swift`.
...
- Implementers never run `Scripts/build-app.sh`, never launch/kill the app, never touch `~/Library/Application Support/Prata` or `.../FluidAudio` (a model conversion writes there concurrently).
...
**Files:** `Sources/PrataCore/PasteService.swift`, `Sources/PrataCore/RecordingController.swift`, `Sources/Prata/AppDelegate.swift` (wire default), `Tests/PrataCoreTests/PasteServiceTests.swift`.
...
**Files:** create `Sources/PrataCore/AppSettings.swift`, `Sources/PrataCore/TriggerKey.swift`, `Sources/PrataCore/TriggerMonitor.swift` (replaces `RightCommandKeyMonitor.swift`, delete it); modify `PushToTalkGesture.swift`, `AppDelegate.swift`, `Package.swift` (re-add `KeyboardShortcuts` from `3.1.0` for custom combos); tests `PushToTalkGestureTests.swift`, `TriggerKeyTests.swift`, `AppSettingsTests.swift`.
...
**Files:** create `Sources/Prata/SettingsView.swift`, `Sources/Prata/SettingsWindowController.swift`, `Sources/PrataCore/LoginItem.swift`; modify `AppDelegate.swift`.
...
- SwiftUI `Form`: Trigger (picker of `TriggerKey` display names; when `.customShortcut` show `KeyboardShortcuts.Recorder` for `.prataCustomTrigger`), Mode (segmented: Push to talk / Toggle), Model (picker; Pianissimo disabled with "not installed" when missing), Keep clipboard content (toggle), Launch at login (toggle).
...
**Files:** create `Sources/PrataCore/AudioLevel.swift`, `Sources/Prata/RecordingIndicator.swift` (panel + SwiftUI view); modify `AudioRecorder.swift` (level callback), `RecordingController.swift` (publish level + recording state), `AppDelegate.swift`; test `Tests/PrataCoreTests/AudioLevelTests.swift`.
